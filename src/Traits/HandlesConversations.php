...
trait HandlesConversations
{
     ...
     */
    public function startConversation(Conversation $instance, $sender = null, $recipient = null, $driver = null)
    {
        if (!is_null($recipient) && !is_null($driver)) {
            $this->message = new IncomingMessage('', $sender, $recipient, null, $this->config['bot_id']);
    ...
    {
        $botToken = $instance->getBot()->getDriver()->getConfig()['token'];
        $conversation_cache_time = $instance->getConversationCacheTime();

        $this->cache->put($this->message->getConversationIdentifier($botToken), [
    ...
    public function getStoredConversation($message = null)
    {
        ...
        $botToken = @$this->getDriver()->getConfig()['token'];
        $conversation = $this->cache->get($message->getConversationIdentifier($botToken));
    ...
    public function touchCurrentConversation()
    {
        if (!is_null($this->currentConversationData)) {
            ...
            $botToken = @$this->getDriver()->getConfig()['token'];
            $this->cache->put($this->message->getConversationIdentifier($botToken), $touched, $this->config['config']['conversation_cache_time'] ?? 30);
        }
    }
    ...
    public function removeStoredConversation($message = null)
    {
        ...
        if (isset($conversation['time']) && ($conversation['time'] == $this->currentConversationData['time'])) {
            $botToken = @$this->getDriver()->getConfig()['token'];
            $this->cache->pull($this->message->getConversationIdentifier($botToken));
    ...
    public function loadActiveConversation()
    {
        ...
        Collection::make($this->getMessages())->reject(function (IncomingMessage $message) {
        ...
        })->filter(function (IncomingMessage $message) use ($botToken) {
            return $this->cache->has($message->getConversationIdentifier($botToken)) || $this->cache->has($message->getOriginatedConversationIdentifier($botToken));
        })->each(function ($message) use ($botToken) {
            ...
            if ($convo['conversation']->stopsConversation($message) === true) {
                $this->cache->pull($message->getConversationIdentifier($botToken));
